...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(500.0, 250.0),  # All numbers are approximate.
                rg.Point(327.3, 487.8),
                rg.Point(47.7, 396.9),
                rg.Point(47.7, 103.1),
                rg.Point(327.3, 12.2)]
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 9
    circle.fill_color = 'white'
    polygon(window, circle, 99, 'green', 10)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(100):
        c = rg.Line(points[k%len(points)], points[(k+1)%len(points)])
        c.color = color
        c.thickness = thickness
        c.attach_to(window)
    window.render()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(100):
        c = rg.Line(points[k%len(points)], points[(k+hops_to_next_point)%len(points)])
        c.color = color
        c.thickness = thickness
        c.attach_to(window)
    window.render()
